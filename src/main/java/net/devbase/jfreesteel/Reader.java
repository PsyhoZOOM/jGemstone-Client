...
/**
 * Reader class maintains the connection with the terminal and provides an interface for your code
 * to receive card insertion/removal events.
 *
 * To assign the listeners for the event, pass the object implementing ReaderListener interface to
 * the Reader.addCardListener() method.
 *
 * On card insertion insert() method of the listener will be called passing new EidCard object that
 * can be used to read data from the Serbian eID card. Listener should assume that card is removed
...
public class Reader {
  ...
  /**
   * Add new card listener to be notified on card insertion/removal. Listeners should assume that
   * the card is removed in default state.
   *
  ...
  /**
   * Remove card listener from the list of listeners. Does nothing if the listener is not present in
   * the list.
   *
   ...
   * sizes and what else not... terminal.isCardPresent() always returns false,
   * terminal.waitForCard() and terminals.waitForChange() do not wait, and terminals.list()
   * occasionally causes SIGSEGV!
   *
  ...
  public interface ReaderListener {
    ...
    /**
     * Card is inserted into the reader terminal. Use EidCard object to read data from the eID
     * card.
     *
