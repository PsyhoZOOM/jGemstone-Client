...
public class KorisnikUslugeEditController implements Initializable {
    ...
    public void setData() {
        System.out.println("ID: " + servicesUser.getId() + " PAKET TYPE:" + servicesUser.getPaketType());
        lNazivUsluge.setText(servicesUser.getNazivPaketa());
        if (servicesUser.getPaketType().equals("IPTV") || servicesUser.getPaketType().equals("LINKED_IPTV")) {
            tIdentification.setText(servicesUser.getIPTV_MAC());
        } else if (servicesUser.getPaketType().equals("DTV") || servicesUser.getPaketType().equals("LINKED_DTV")) {
        ...
        if (servicesUser.getEndDate() != null) {
            LocalDate date = LocalDate.parse(servicesUser.getEndDate(), DateTimeFormatter.ofPattern("yyyy-MM-dd"));
