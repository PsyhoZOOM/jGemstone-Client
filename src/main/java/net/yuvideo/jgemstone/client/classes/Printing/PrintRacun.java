...
public class PrintRacun {
    ...
    public void printRacun() {
        ...
        pageLayout = printerJob.getPrinter().createPageLayout(paper, pageOrientation, Printer.MarginType.HARDWARE_MINIMUM);
        ...
        Font font = Font.loadFont(getClass().getResource("/font/roboto/Roboto-Regular.ttf").toExternalForm(), 10);
        Font fontSmall = Font.loadFont(getClass().getResource("/font/roboto/Roboto-Light.ttf").toExternalForm(), 8);
        ...
        for (i = 0; i < userRacun.size() - 1; i++) {
            ...
            cell.setMaxWidth(NAZIV);
            cell.setAlignment(Pos.CENTER);
            cell.setStyle("-fx-border-style: solid; -fx-border-color: black; -fx-border-width: 0 0.2 0.2 1;");
            ...
            cell.setMaxWidth(KOLICINA);
            cell.setAlignment(Pos.CENTER);
            cell.setStyle("-fx-border-style: solid; -fx-border-color: black; -fx-border-width: 0 0.2 0.2 0;");
            row.getChildren().add(cell);
            ...
            cell.setMinWidth(UKUPNO);
            cell.setMaxWidth(UKUPNO);
            cell.setStyle("-fx-border-style: solid; -fx-border-color: black; -fx-border-width: 0 1 0.2 0;");
            row.getChildren().add(cell);
        ...
        LocalDate zaPrDate = LocalDate.parse(racun.getZaPeriod() + "-01", DateTimeFormatter.ofPattern("yyyy-MM-dd"));
        String stDate = zaPrDate.withDayOfMonth(1).format(DateTimeFormatter.ofPattern("dd.MM"));
        String lstDate = zaPrDate.withDayOfMonth(zaPrDate.lengthOfMonth()).format(DateTimeFormatter.ofPattern("dd.MM"));

        row = new HBox();
        Label zaP = new Label(String.format("Za period: \n %s-%s.%s", stDate, lstDate, zaPrDate.format(DateTimeFormatter.ofPattern("yyyy"))));
        zaP.setFont(font);
        cell = new HBox(zaP);
        row.getChildren().add(cell);
        ...
        row = new HBox();
        Label mestoDatumIzdavanja = new Label(String.format("Mesto i datum izdavanja računa: \n Petrovac na Mlavi %s", LocalDate.now().format(DateTimeFormatter.ofPattern("dd-MM-yyyy"))));
        ...
        row = new HBox();
        Label datumPrometa = new Label(String.format("Datum prometa: \n %s", zaPrDate.withDayOfMonth(zaPrDate.lengthOfMonth()).format(DateTimeFormatter.ofPattern("dd-MM-yyyy"))));
        ...
        row = new HBox();
        Label rokZaPlacanje = new Label(String.format("Rok za plaćanje: \n %s", LocalDate.now().plusDays(30).format(DateTimeFormatter.ofPattern("dd-MM-yyyy"))));
        ...
        //QRCODE
        File fImg = net.glxn.qrgen.javase.QRCode.from(racun.getSifraKorisnika()).withCharset("CP1250").file();
        ...
        row = new HBox();
        Label ukupnoZaUplatu = new Label(String.format("Ukupno: %s", df.format(racun.getUkupnoUkupno())));
        ...
        row = new HBox();
        Label prethodniDug = new Label(String.format("Prethodni dug: %s", df.format(racun.getPrethodniDug())));
        ...
        row = new HBox();
        Label UKUPNO_ZA_UPLATU = new Label(String.format("Ukupno za Uplatu: %s", df.format(racun.getUkupanDug())));
        ...
        //RACUN ADRESA
        Text adresaRacunaKorisnika = new Text(String.format("%s\n%s\n%s", racun.getIme(), racun.getAdresaRacuna(), racun.getMestoRacuna()).toUpperCase());
        ...
        String rokZaPlacanjeRacun = LocalDate.now().plusDays(30).format(DateTimeFormatter.ofPattern("dd.MM.yy"));

        String pozivNaBroj = String.format("%s/%s", racun.getSifraKorisnika(), LocalDate.parse(racun.getZaPeriod() + "-01", DateTimeFormatter.ofPattern("yyyy-MM-dd")).format(DateTimeFormatter.ofPattern("MM")));
        ...
        Scene scene = new Scene(anchorPane, pageLayout.getPrintableWidth(), pageLayout.getPrintableHeight());
